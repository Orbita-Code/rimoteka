...
| **KS-1** | prvi izbor ćirilice na telefonu otvarao uputstvo 582 px i sakrivao polje → rašireno samo uz fizičku tastaturu (`hover:hover and pointer:fine`) | 58 |
| **MB-1..7, M-2, U-4** | tajmer igre van kadra (57); reč u traci 42×18 → 44 px i ≥12 px; putanja 44 px; resize bez greške na stranama bez brojača; položen telefon = telefon; baner kompaktan na ≤600 px visine; kružić slogova 12 px; na dodirnim ekranima tabovi/filteri/prekidači/kapsule ≥44 px i razmak 8 px | 58 |
| **PR-1..7, A-1, A-2** | prijava: „Zatvori", Tab kruži, imenovana napomena, radiogroup, fokus se vraća; kartica i tastaturom na ≤560 px + `aria-haspopup/expanded`; oblačić `role=status` + kontrast ≥4,5; igra: `aria-live` na reči i poruci, etiketa polja, `aria-pressed` na opcijama, **dugme „Pauza"**, niz ≥4,5; `aria-pressed` na pismu/temi/filteru; `lang` sr-Cyrl/sr-Latn; `aria-label` na naslovu pesme; okvir fokusa 3 px na poljima; Escape gasi kucanje | 58 |
| **G-4, UI-5..8, L-2..5, K-2** | mreža ≠ „nema objašnjenja"; `&slog=` ostaje u adresi taba; logo vraća kanonikal/naslov; strane bez igre bez upozorenja; pretraga po završetku pamti `?kraj=&nacin=`; zabranjene reči sa kvačicama (smeće, đubre, pička…) + `đubrad`; `zarđati`=4, `žanr`=1 u oba sistema; komentar redosleda ispravljen; „Beograd" u statusu; filter slogova je PODSKUP prikazanih | 58 |
| **U-1, U-2, U-3** | dodir na dugme dok se kuca ne gasi tastaturu; ceo panel rima drži fokus; klik na ivicu kapsule fokusira reč | 58 |
| **PF-4, PF-5, P-3** | komad obrade rečnika 20.000 → 5.000, rangiranje u komadima; `defer` na `app.js` (sve strane); fontovi sa `?v=2` | mereno sledećim auditom |
| **SEO-1, SEO-4..8, B-4, BZ-5** | „Kopiraj sve rime" kopira ceo spisak; 404 bez Google fontova; `/index.html` i `/RIME-ZA/` → 301; tematske strane linkuju na strane reči (7 linkova); opis dečje strane 156 znakova; robots blokira `&decji=`, `&tab=`, `?kraj=`; HSTS `includeSubDomains`; SW kešira samo početnu (v8) | 58 (produkcija) |
| **B-1, B-3, BZ-2, BZ-3** | sanduče: plafon 60/sat + dedup 10 min; 10 pogrešnih ključeva → 429; ključ poređen bez merenja vremena; bez dužina lozinke u odgovoru (worker `73b53eff`) | curl uživo (agent) |
| **SJ-2..4, SJ-7, SJ-10..12** | srpski navodnici u 2 HTML + 5 poruka + 283 objašnjenja; 15.912 objašnjenja bez ponovljene reči na kraju; primer stiha pod navodnicima; 20 dugačkih crta u komentarima; zarez pred „i" u komentaru | 34 (zarez), 53 (definicije) |
| **TP-5..8, TP-10, TP-11** | BLOCKED isti u oba sistema; slogovi+ključ isti za 2.008 reči; sirovi vs JS kanonikal zapisan; Enter u pretrazi; sistemska tamna tema; Pro/paywall kod i modal uklonjeni; čuvar „bar 900 provera" | 58, 13 |
| **PF-1** | prva rima na sporoj vezi 7,1 s (ceo rečnik 666 KB gzip) → **rečnik po kantama** (`build/kante.py` → `public/kante/<xy>.txt`, 378 kanti, najveća 176 KB / medijana <1 KB): za ukucanu reč ili `?rec=` skine se samo njena kanta i rime izlaze za ~0,3 s; ceo rečnik stiže u pozadini i spisak se ne prepisuje dok ne stigne i učestalost (isti redosled) | 59: 5 reči – spisak iz kante = spisak iz celog rečnika; rime pre `WORDS` |
| **PF-3** | `app.js` 105 KB gzip → **minifikovan** `app.min.js` (45 KB gzip, `scripts/minifikuj.mjs`, pravi ga `osvezi-verzije-podataka.mjs`), servira se POD ISTOM adresom `/app.js` (nginx `alias`, lokalni server isto); izvor sa komentarima ostaje | 59: otisak u min = otisak app.js; `/app.js` daje min; nginx-provera |
| **P-2** | `frekvencija.json` (957 KB gzip) na svakoj poseti → skida se na prvu potrebu (rime, kockica, beležnica) ili posle 6 s; kockica čeka; prikaz se osveži kad stigne (K1) | 59 |
| **SEO-2** | sirovi kanonikal `?rec=` uvek `/` → nginx `map $arg_rec` + `sub_filter` (`nginx-kanon-rec.map`, 2.410 ključeva: ASCII slug i reč sa kvačicama) upiše `/rime-za/<reč>/` u SIROVI HTML za 1.989 reči sa stranom; ostalo `/` | 59 (produkcija), nginx-provera 3b |
| **BZ-4** | CSP `script-src *.googletagmanager.com` → tačan host `www.googletagmanager.com` | 59 (produkcija) |
| **TP-9** | 8 lažno zelenih provera pooštreno po sadržaju (pretraga: svi rezultati na traženi niz; kockica: reč iz rečnika sa učestalošću; prekidač pisma stvarno prebacuje; 404 forma vodi na `/?rec=`; label ima tekst; filter 1 slog svaka 1; fontovi sa `?v=`) | same provere |
...
| „glada" kao oblik reči „glad" (ne postoji; objašnjenje iz LLM talasa nad narodnim oblikom iz OpenSubtitles korpusa) | obrisano iz `reci.txt` i `definicije.json` po naredbi vlasnice. Isti obrazac rešen po njoj: 10.09. `koba`, `peća`, `radosta`, `vlasa`, `zoba`, `zvera`; 11.09. posle skena celog rečnika još `divita`, `preveza`, `priona`, `progovora`, `smoka`, `zasita`, `šarafa` i ćelava latinica `bubasvaba`, `deriste`, `gace`, `kocija`, `obarac`, `spageti`, `virsla` | — |
...
| D-3 | „prozorče sa opcijama iskače čim se pređe na reč, prekriva reči iznad; treba mala zadrška" | tačno: traka se otvarala posle 110 ms | **odluka vlasnice 08.09. uveče: kartica se otvara SAMO klikom/dodirom (i tastaturom), prelazak miša je ne otvara uopšte**; legenda: „Klikni na reč i otvoriće se kartica…" | 56 (D-3), 46 (legenda) |
...
| I-6 ZATVORENO | bazen reči koje igra zadaje = 8.000 najčešćih iz veb-korpusa (novine): 304 nepunoznačne (koji, kao, ali, nije, kojima…), 432 priloga, 678 predugih, 31 ime; hrvatskih 0, zastarelih 0 | u partiji od 10 reči dete je dobijalo bar jednu nepunoznačnu u 32 % partija | **odluka vlasnice 08.09.:** igra zadaje samo imenice, glagole i prideve od 2–4 sloga → `public/igra-reci.json` (6.634 reči, `scripts/igra-reci-napravi.py`) | igra-kao-covek (svaka zadata reč mora biti iz spiska, 2–4 sloga) |
| I-7 NOVO | režim „tri rime za jednu reč" i odgovor glasom (odluke vlasnice 08.09.) | – | urađeno; logo 224 px (oštar na iPhone-u); dugmad reda za unos: na 320 px polje u svoj red | igra-kao-covek (tri rime, glas, bez glasa), skener tamne |
...
| **MOB-8** (vlasnica, 08.09. uveče, slika) | futer na generisanim stranama „kilometarski": reči razvučene sa „·" i line-height 1,9, na telefonu svaki link 44 px | stari futer (`FOOTER_TMPL`) nikad nije dobio izgled početne (`futer-v2`) | reči kao pilule u ravnim redovima bez tačkica, natpis u svom redu, isti izgled kao početna; na telefonu 44 px ali zbijeno | pun test (strane), skener tamne |
| **MOB-7** (vlasnica, 08.09. uveče, „ogroman bag") | Enter za novi red usred pesme na telefonu baca na POSLEDNJI red beležnice | u `drziKursorIspodTrake` prazan red posle Entera nema pravougaonik, pa je kao zamena uziman pravougaonik RODITELJA – a to je CEO editor; strana je skrolovala do njegovog dna (WebKit: 165 px) | red se računa od `<br>` ispred kursora; ako ni to ne može – ne pomera se ništa. Nikad pravougaonik editora. | motori (sva 3 motora: pomak 0 px), 56 |
