...
await p.mouse.move(5, 5); await p.locator('#rimeResults .chip').first().click(); await p.waitForTimeout(350); upisi('/ traka', await p.evaluate(MERI));
...
await p.mouse.move(5, 5); await p.locator('#rimeResults .chip').nth(1).click(); await p.waitForTimeout(350); await p.click('.chip-actions .ca-btn[data-act="prijavi"]').catch(() => {}); await p.waitForTimeout(300); upisi('/ prijava', await p.evaluate(MERI)); await p.keyboard.press('Escape');
