...
/* PF-1 (22.09.2026): rime se prvo iscrtaju iz KANTE, pa ih ceo rečnik + učestalost iscrtaju PONOVO – stari
   elementi tada nestanu iz DOM-a. Lokator uzet pre toga puca sa „Element is not attached" (pao lanac protiv
   produkcije 22.09.). Zato se pre hvatanja kapsule čeka da rečnik i učestalost stignu i prikaz se smiri. */
const cekajMirneRime = async (p, ms = 400) => {
...
async function main() {
  ...
  try {
    ...
    /* Naslov /slogovi/ se menjao dvaput u dva dana (4e978f793d nov naslov, edff99300d
       povratak na stari, paralelna sesija). Provera zato gleda samo stabilni deo:
       ...
       drugi tab (koji drži stariju verziju) otkuca jedan jedini znak. */
    {
      ...
      await ctx16.addInitScript(() => { try { localStorage.setItem('rimoteka_interno', '1'); } catch (e) {} });
    ...
    sek('\n19c) PERFORMANSE – rečnik se ne skida bez potrebe, pretraga čeka rečnik (V5)');
    {
      ...
      // kanta putuje mrežom (na produkciji ~0,3 s) – čeka se do 2,5 s, ali se broji DOK JE REČNIK JOŠ PRAZAN
      await p19d.waitForFunction(() => document.querySelectorAll('#rimeResults .chip .word').length > 0 || WORDS.length > 0, null, { timeout: 2500 }).catch(() => {});
      const odmah = await p19d.evaluate(() => WORDS.length > 0 ? -1 : [...document.querySelectorAll('#rimeResults .chip .word')].filter(e => !e.closest('.syn-card')).length);
      await cekajMirneRime(p19d, 600);
      const posle = await p19d.evaluate(() => [...document.querySelectorAll('#rimeResults .chip .word')].filter(e => !e.closest('.syn-card')).length);
      ok('V5 pretraga pre učitanog rečnika: rime iz kante stižu ODMAH (pre rečnika), a kad rečnik stigne spisak ostaje (isti broj)', odmah > 5 && posle > 5 && posle === odmah, `odmah=${odmah} posle=${posle} (−1 = rečnik stigao pre kante)`);
       ...
       ima visinu. Ni jedno srce nikad nije bilo kliknuto. */
    {
      ...
      await p20.locator('#rimeResults .chip').first().click();   // 08.09.: kartica se otvara klikom
      await pauza(350);
          ...
          ako su sva tri loša – kvar je stvaran i ne prolazi. */
    {
      for (const put of ['/', '/rimovanje-reci/']) {
        ...
        for (let pokusaj = 1; pokusaj <= 3; pokusaj++) {
          ...
          await ctx24.addInitScript(() => { try { localStorage.setItem('rimoteka_interno', '1'); } catch (e) {} });
       ...
       Ova provera je puštena protiv starog koda i pala je. */
    {
      ...
      await ctx28.addInitScript(() => { try { localStorage.setItem('rimoteka_interno', '1'); } catch (e) {} });
      ...
      for (const put of ['/', '/pisanje-pesama/']) {
        ...
        const c30 = await p30.evaluate(async (kbH) => {
          ...
          return {
            ...
            vrhTastature: nova,
            panelDno: Math.round(p.bottom), panelVisina: Math.round(p.height), panelVrh: Math.round(p.top),
            /* red sa kursorom (poslednji red pesme) mora da bude ISPOD trake i iznad tastature (08.09.2026) */
            kursorRed: (() => { const ed = document.getElementById('noteEditor'); const zadnji = [...ed.childNodes].reverse().find(n => n.textContent && n.textContent.trim()); const e = zadnji && zadnji.nodeType === 1 ? zadnji : ed; const r = e.getBoundingClientRect(); return { top: Math.round(r.top), bottom: Math.round(r.bottom) }; })(),
        ...
        /* Od 08.09.2026 traka stoji na VRHU vidljivog ekrana (iOS je traku „iznad tastature" crtao na sredini
           ekrana, preko reda koji se kuca – dve slike vlasnice). Red sa kursorom je između trake i tastature. */
        ok(`${put} · traka sa rimama je na VRHU vidljivog ekrana, ne preko teksta`, c30.panelVrh <= 1, `vrh trake ${c30.panelVrh} px`);
        ok(`${put} · red u kome se kuca vidi se ISPOD trake i IZNAD tastature`, c30.kursorRed.top >= c30.panelDno - 1 && c30.kursorRed.bottom <= c30.vrhTastature + 1, `red ${c30.kursorRed.top}–${c30.kursorRed.bottom}, traka do ${c30.panelDno}, tastatura od ${c30.vrhTastature}`);
    ...
    sek('\n32) REČ NAVEDENA KAO RIMA MORA DA BUDE RIMA (nalaz T2)');
    {
      ...
      const c32 = await browser.newContext();
      await c32.addInitScript(() => { try { localStorage.setItem('rimoteka_interno', '1'); } catch (e) {} });
    ...
    sek('\n39) REDOSLED RIMA – strana i alat moraju da daju ISTO (nalaz K1)');
    {
      ...
      await ctx39.addInitScript(() => { try { localStorage.setItem('rimoteka_interno', '1'); } catch (e) {} });
    ...
    sek('\n45) SLOGOVI – veliko početno slovo broji se isto kao malo (prijava korisnika 06.09.2026)');
    {
      ...
      await c45.addInitScript(() => { try { localStorage.setItem('rimoteka_interno', '1'); } catch (e) {} });
    ...
    sek('\n46) „PRIJAVI GREŠKU" – dugme, prozorčić i slanje u sanduče (06.09.2026)');
    {
      ...
      await p46i.goto(BASE + '/?rec=nepostojan', { waitUntil: 'domcontentloaded' }); await p46i.waitForSelector('#rimeResults .chip', { timeout: 20000 }); await cekajMirneRime(p46i, 800);
      await p46i.locator('#rimeResults .chip').first().click(); await pauza(350); await p46i.click('.chip-actions .ca-btn[data-act="prijavi"]'); await pauza(300); await p46i.click('.prijava-posalji'); await pauza(1000);
      ...
      await cekajMirneRime(p46b, 1200);
      ...
      ok('računar · legenda je tačno tekst vlasnice (08.09.2026 uveče: „Klikni na reč…")', /^2\s*Broj u kružiću označava koliko slogova ima data reč\.\s*Klikni na reč i otvoriće se kartica sa njenim objašnjenjem, a tu možeš i da je sačuvaš, nađeš rime za nju, kopiraš je ili prijaviš kao grešku\.$/.test(kapsule.legenda.replace(/\s+/g, ' ').trim()), kapsule.legenda);
      await p46b.evaluate(() => document.querySelector('#rimeResults .chip').scrollIntoView({ block: 'center' })); await pauza(300);
      await p46b.locator('#rimeResults .chip').first().click(); await pauza(350);
      ...
      await p46b.locator('#rimeResults .chip').nth(2).click(); await pauza(350);
      ...
      await p46b.locator('#rimeResults .chip').nth(1).click(); await pauza(350);
    ...
    sek('\n47) BANER ZA KOLAČIĆE – napredni Consent Mode: gtag uvek, pristanak „denied" dok se ne prihvati (12.09.2026)');
    {
      /* Odluka vlasnice 06.09.2026: baner; 12.09.2026: NAPREDNI Consent Mode – gtag.js se učitava
         ...
         „Prihvati sve" ide `consent update` → „granted". Odbijanje kroz „Podesi" ostavlja „denied".
      ...
      // (a) telefon: baner se vidi, gtag JESTE učitan ali pristanak „denied", polje za reč je slobodno
      const stanje = () => ({ gaSkripta: !!document.querySelector('script[src*="googletagmanager"]'), pristanak: window.rimotekaKolacici && window.rimotekaKolacici.pristanak(),
        podrazumevano: (window.dataLayer || []).some(a => a[0] === 'consent' && a[1] === 'default' && a[2] && a[2].analytics_storage === 'denied' && a[2].ad_storage === 'denied'),
        poslednjiUpdate: ((window.dataLayer || []).filter(a => a[0] === 'consent' && a[1] === 'update').pop() || [])[2] });
      ...
      const s1 = await p.evaluate(stanje);
      ok('telefon · bez pristanka gtag.js JESTE učitan (napredni režim), ali pristanak je „denied" i podrazumevano je sve odbijeno', !!b1 && b1.gaSkripta === true && s1.pristanak === 'denied' && s1.podrazumevano === true && !s1.poslednjiUpdate, JSON.stringify(s1) + ` zahteva ${ga.length}`);
      ...
      const s2 = await p.evaluate(stanje);
      ok('telefon · „Prihvati sve" sakrije baner, upiše odluku i pošalje consent update „granted"', !b2.baner && /"analitika":true/.test(b2.odluka || '') && b2.gaSkripta && ga.length >= 1 && s2.pristanak === 'granted' && !!s2.poslednjiUpdate && s2.poslednjiUpdate.analytics_storage === 'granted', JSON.stringify(b2) + ' ' + JSON.stringify(s2) + ` zahteva ${ga.length}`);
      await p.reload({ waitUntil: 'domcontentloaded' }); await pauza(1200);
      ok('telefon · posle osvežavanja baner se ne vraća, gtag učitan, pristanak odmah „granted"', await p.evaluate(() => !document.querySelector('.kolacici') && !!document.querySelector('script[src*="googletagmanager"]') && window.rimotekaKolacici.pristanak() === 'granted'));
      await c.close();
      // (b) računar: „Podesi" → prekidač isključen → „Sačuvaj" → pristanak ostaje „denied", baner nestaje; futer „Kolačići" vraća baner
      ...
      const s4 = await p.evaluate(stanje);
      ok('računar · „Sačuvaj" sa isključenim merenjem: baner nestaje, odluka „ne", pristanak ostaje „denied" (nijedan update „granted")', !b4.baner && /"analitika":false/.test(b4.odluka || '') && s4.pristanak === 'denied' && !(s4.poslednjiUpdate && s4.poslednjiUpdate.analytics_storage === 'granted'), JSON.stringify(b4) + ' ' + JSON.stringify(s4) + ` zahteva ${ga.length}`);
    ...
    sek('\n49) POPRAVKE IZ AUDITA 07.09. – tastatura u traci, tajmer prijave, 320 px, filter, omiljene');
    {
      ...
      // A2
      await p49.mouse.move(5, 5); await p49.locator('#rimeResults .chip').nth(0).click(); await pauza(350);
      ...
      await p49.mouse.move(5, 5); await pauza(300); await p49.locator('#rimeResults .chip').nth(3).click(); await pauza(350);
      ...
      // S-01
      await p49.locator('#rimeResults .chip').nth(1).click(); await pauza(350);
      ...
      for (const cyr of [false, true]) {
        ...
        await p.fill('#rimeInput', 'ljubav'); await p.evaluate(() => document.getElementById('rimeBtn').click());
        await p.waitForFunction(() => document.querySelectorAll('#rimeResults .chip').length > 5, null, { timeout: 60000 }); await cekajMirneRime(p, 500);
    ...
    sek('\n50) SVE STRANE – brza provera svih strana iz sitemapa (fajlovi), i uzorak na sajtu');
    {
      ...
      if (LOKALNO) {
        ...
        /* Dugačka crta (em dash, U+2014) je ZABRANJENA na sajtu (odluka vlasnice 08.09.2026): koristi se kratka (–). */
        const saCrtom = ['index.html', 'app.js', 'ga-init.js', '404.html', 'style.css', 'sw.js', 'manifest.json'].filter(f => fs.readFileSync(path.join(ROOT, 'public', f), 'utf8').includes('\u2014'));
        let stranaSaCrtom = 0; for (const u of adrese) { const f = path.join(ROOT, 'public', u.replace('https://rimoteka.com', '').replace(/\/$/, ''), 'index.html'); if (fs.existsSync(f) && fs.readFileSync(f, 'utf8').includes('\u2014')) stranaSaCrtom++; }
    ...
    sek('\n51) POPRAVKE IZ AUDITA 07.09. – DRUGI KRUG: rečnik sa HTML-om, igra preživi stranu, 44 px, baner, oblačić, statičke strane');
    {
      ...
      // ---------- S-13, S-16: baner – prvi u telu, Escape sklanja bez odluke, ne zaklanja fokus ----------
      {
        ...
        /* 12.09.2026 napredni Consent Mode: gtag JESTE učitan i bez odluke, ali pristanak ostaje „denied". */
        const e = await p.evaluate(() => ({ baner: !!document.querySelector('.kolacici'), odluka: localStorage.getItem('rimoteka_kolacici'), sp: document.documentElement.style.scrollPaddingBottom, pristanak: window.rimotekaKolacici && window.rimotekaKolacici.pristanak() }));
        ok('S-16 · Escape sklanja baner BEZ odluke, pristanak ostaje „denied" (odbijanje ostaje kroz „Podesi")', !e.baner && e.odluka === null && e.sp === '' && e.pristanak === 'denied', JSON.stringify(e));
      ...
      // ---------- S-15 + N-R1 + N-04 + N-05 + N-01 + N-02 + N-03 (računar) ----------
      {
        ...
        await p.mouse.move(5, 5); await p.locator('#rimeResults .chip').nth(0).click(); await pauza(350);
      ...
      // ---------- S-08 + S-09: statička strana /rime-za/ – traka nad reči, ćirilica futera i naslova kapsula ----------
      {
        ...
        await p.mouse.move(5, 5); await p.locator('.res-group .results .chip').first().click(); await pauza(350);
...
print(len(ws), len(bad), ' '.join(bad[:6]))"`, { cwd: ROOT, encoding: 'utf8' }).trim().split(' ');
      ...
      /* Branik za 23 zabranjene reči (audit 12.09.2026, nalaz T-1): regeneracija rečnika istim
         postupkom kojim su i nastale (LLM talas nad OpenSubtitles korpusom) ne sme ih tiho vratiti.
         Ova provera pada čim se ijedna vrati; kao red u `reci.txt`, ključ u `definicije.json` ili
         u ijednom fajlu u `public/definicije/` (splitovi i spisak) ili u `matica.json`.
         Ista lista i obrazac: `AUDIT/2026-09-12-audit.md`; pravilo brisanja: CLAUDE.md tačka 4. */
      if (LOKALNO) {
        const fs54 = await import('node:fs');
        const ZABRANJENE_54 = ['glada','bica','dionica','koba','peća','radosta','vlasa','zoba','zvera','divita','preveza','priona','progovora','smoka','zasita','šarafa','bubasvaba','deriste','gace','kocija','obarac','spageti','virsla'];
        const reci54 = fs54.readFileSync(ROOT + '/public/reci.txt', 'utf8').split('\n');
        const uRecima = ZABRANJENE_54.filter(w => reci54.includes(w));
        const pogodci = [...uRecima];
        const skeniraj = (putanja, oznaka) => {
          if (!fs54.existsSync(putanja)) return;
          const s = fs54.readFileSync(putanja, 'utf8');
          for (const w of ZABRANJENE_54) if (s.includes(`"${w}"`)) pogodci.push(`${oznaka}:${w}`);
        };
        skeniraj(ROOT + '/public/definicije.json', 'definicije');
        skeniraj(ROOT + '/public/matica.json', 'matica');
        for (const f of fs54.readdirSync(ROOT + '/public/definicije')) {
          if (f.endsWith('.json')) skeniraj(ROOT + '/public/definicije/' + f, f.replace('.json', ''));
        }
        ok(`branik · 23 zabranjene reči nigde nema (reci.txt, definicije, splitovi, matica)`, pogodci.length === 0, pogodci.slice(0, 8).join(', '));
      }
    }
    ...
    sek('\n55) RUPE IZ AUDITA (TODO 24) + HUB – ćirilica×telefon, dečji×telefon, 320 px, rubni unosi, pretraga u spisku');
    {
      ...
      // brojač slogova na telefonu: dodir u PRAZNO polje ne sme da odnese stranu na dno polja (vlasnica, treći put 08.09.2026)
      {
        const c = await browser.newContext({ viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true });
        await c.addInitScript(() => localStorage.setItem('rimoteka_interno', '1'));
        const p = ojacajStranu(await c.newPage());
        await p.goto(BASE + '/slogovi/', { waitUntil: 'domcontentloaded' }); await pauza(600);
        await p.tap('#sylInput');
        const r = await p.evaluate(async (kbH) => { const t = document.getElementById('sylInput'); const pre = { scrollY: window.scrollY, top: Math.round(t.getBoundingClientRect().top) };
          const VV = window.visualViewport; const nova = window.innerHeight - kbH; Object.defineProperty(VV, 'height', { get: () => nova, configurable: true }); Object.defineProperty(VV, 'offsetTop', { get: () => 0, configurable: true }); VV.dispatchEvent(new Event('resize')); window.dispatchEvent(new Event('resize'));
          await new Promise(r => setTimeout(r, 400));
          return { pre, posle: { scrollY: window.scrollY, top: Math.round(t.getBoundingClientRect().top) }, vidljivo: nova }; }, 336);
        ok('brojač slogova · telefon: dodir u prazno polje pomeri stranu najviše koliko treba da se vidi prvi red (≤ 120 px), vrh polja ostaje u kadru', r.posle.scrollY - r.pre.scrollY <= 120 && r.posle.top > 60 && r.posle.top < r.vidljivo, JSON.stringify(r));
        await c.close();
      }
    ...
    sek('\n56) PRIJAVE KORISNIKA DRAGANA M. (08.09.2026) + REČ DANA + PROBNA ZAMENA STRANA');
    {
      ...
      const hov = await p.evaluate(() => { const t = document.querySelector('.chip-actions'); return !!(t && !t.hidden); });
      await p.locator('#rimeResults .chip').nth(2).click(); await pauza(350);
      const klik = await p.evaluate(() => { const t = document.querySelector('.chip-actions'); return !!(t && !t.hidden) && t.querySelectorAll('.ca-btn').length === 5; });
      ok('D-3 · kartica nad reči se NE otvara prelaskom miša (650 ms), a otvara se klikom (5 radnji)', !hov && klik, `prelazak: ${hov}, klik: ${klik}`);
      await p.keyboard.press('Escape');
      await p.click('#azbukaToggle'); await pauza(500);
      /* Azbuka se proverava ZNAK-PO-ZNAK sa digrafima kao jednim slovom (audit 12.09.2026, nalaz T-2):
         ranije je provera gledala samo prvo slovo, pa su redosledi od drugog znaka i digrafi
         (lj, nj, dž) prolazili pogrešno. Kopiju reda sortiramo istom funkcijom `porediAzbuka`
         iz aplikacije i poredimo sa prikazanim redom; posebno proveravamo parove sa digrafima. */
      const az = await p.evaluate(() => {
        const g = [...document.querySelectorAll('#rimeResults .res-group')].map(x => [...x.querySelectorAll('.chip')].map(c => c.dataset.w || ''));
        const sveSortirano = g.every(r => { const kopija = [...r].sort(porediAzbuka); return r.length === kopija.length && r.every((w, i) => w === kopija[i]); });
        const digrafi = porediAzbuka('lubim', 'ljubav') < 0 && porediAzbuka('ljubav', 'moka') < 0 && porediAzbuka('lubim', 'moka') < 0 && porediAzbuka('duo', 'džak') < 0 && porediAzbuka('džak', 'đak') < 0;
        return { grupe: g.length, sveSortirano, digrafi, upamceno: localStorage.getItem('rimoteka_azbuka') };
      });
      ok('D-1 · „po azbuci": grupe poređane znak-po-znak (digrafi lj/nj/dž jedno slovo), izbor upamćen', az.grupe >= 1 && az.sveSortirano && az.digrafi && az.upamceno === '1', JSON.stringify(az));
      await p.evaluate(() => localStorage.setItem('rimoteka_script', 'cyr')); await p.reload({ waitUntil: 'domcontentloaded' });
      await p.waitForFunction(() => document.querySelectorAll('#rimeResults .chip').length > 5 && typeof RANK !== 'undefined' && RANK.get('gubav') < 0, null, { timeout: 180000 }); await pauza(500);
      const azc = await p.evaluate(() => {
        const g = [...document.querySelectorAll('#rimeResults .res-group')].map(x => [...x.querySelectorAll('.chip')].map(c => (c.querySelector('.word') || c).textContent.trim()));
        const sveSortirano = g.every(r => { const kopija = [...r].sort(porediAzbuka); return r.length === kopija.length && r.every((w, i) => w === kopija[i]); });
        // ћirilični digrafi su pojedina slova; kontrolni parovi: л < љ, т < ћ, ц < ч < џ < ш
        const parovi = porediAzbuka('лава', 'љубав') < 0 && porediAzbuka('лава', 'мама') < 0 && porediAzbuka('тара', 'ћилиба') < 0 && porediAzbuka('целa'.replace('a', 'а'), 'чело') < 0 && porediAzbuka('чело', 'џak'.replace('ak', 'ак')) < 0 && porediAzbuka('џin'.replace('in', 'ин'), 'шала') < 0;
        return { sveSortirano, parovi, prvih: g[0].slice(0, 4) };
      });
      ok('D-1 · u ćirilici redosled ide po azbuci znak-po-znak (л пре љ, ћ после т, ч пре џ пре ш)', azc.sveSortirano && azc.parovi, JSON.stringify(azc));
...
`], { cwd: ROOT, maxBuffer: 50 * 1024 * 1024 }).toString());
        ...
        // SEO-2 (22.09.): na produkciji nginx `sub_filter` već u sirovom HTML-u upiše stranu reči; lokalni test-server nema sub_filter pa daje početnu
        const sirovoOk = /^https:/.test(BASE) ? /rel="canonical" href="https:\/\/rimoteka\.com\/rime-za\/ljubav\/"/.test(sirovo) : /rel="canonical" href="https:\/\/rimoteka\.com\/"/.test(sirovo);
        ok('TP-6 · ?rec=: sirovi kanonikal (produkcija: strana reči kroz nginx, SEO-2; lokalno: početna), a posle JS strana reči', sirovoOk && /\/rime-za\/ljubav\/$/.test(kan), kan);
        ...
        await cekajMirneRime(p, 300);
        await p.evaluate(() => document.querySelector('#rimeResults .chip .word').click()); await pauza(300);
        await p.evaluate(() => document.querySelector('.chip-actions [data-act="prijavi"]').click()); await pauza(400);
        ...
        await cekajMirneRime(p, 0); await p.evaluate(() => { rimeInput.value = 'ljubav'; doRhymes(); }); await pauza(400);
      ...
      // D) telefon: KS-1, PR-2, MB-2/MB-4/M-2 mete, MB-5, U-2, A-2, MB-3, A-1, TP-8 sistemska tamna
      {
        ...
        await p.evaluate(() => { rimeInput.value = 'ljubav'; doRhymes(); }); await cekajMirneRime(p, 500);   // učestalost stiže na potrebu (P-2) i ponovo iscrta kapsule – fokus ide na SMIREN prikaz
