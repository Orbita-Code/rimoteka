...
try {
  for (const ime of MOTORI) {
    ...
    for (const cyr of [false, true]) {
      ...
      const b = await p.evaluate(() => { const box = document.getElementById('noteRhymes'); const r = box.getBoundingClientRect(); const ed = document.getElementById('noteEditor'); const zadnji = [...ed.childNodes].reverse().find(n => n.textContent && n.textContent.trim()); let z; if (zadnji && zadnji.nodeType === 1) z = zadnji.getBoundingClientRect(); else if (zadnji && zadnji.nodeType === 3) { const rr = document.createRange(); rr.setStart(zadnji, Math.max(0, zadnji.data.length - 1)); rr.setEnd(zadnji, zadnji.data.length); z = rr.getBoundingClientRect(); } else z = ed.getBoundingClientRect();   /* 22.09.2026: red sa kursorom je POSLEDNJI RED teksta, ne ceo editor (min-height 240 px je lažno padao) */
